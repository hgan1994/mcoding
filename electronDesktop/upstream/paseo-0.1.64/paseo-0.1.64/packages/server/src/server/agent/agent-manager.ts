...
export class AgentManager {
  ...
  private async dispatchSessionEvent(
    ...
    const eventWithForegroundTurn = this.attachForegroundTurnIdIfMissing(agent, event);
    const turnId = (eventWithForegroundTurn as { turnId?: string }).turnId;
    ...
    const shouldNotifyWaiters = await this.handleStreamEvent(agent, eventWithForegroundTurn);
    ...
    for (const waiter of matchingWaiters) {
      waiter.callback(eventWithForegroundTurn);
      if (isTurnTerminalEvent(eventWithForegroundTurn)) {
        this.settleForegroundTurnWaiter(waiter);
      }
    }
  }

  private attachForegroundTurnIdIfMissing(
    agent: ActiveManagedAgent,
    event: AgentStreamEvent,
  ): AgentStreamEvent {
    ...
    if ((event as { turnId?: string }).turnId || !agent.activeForegroundTurnId) {
      return event;
    }
    return {
      ...event,
      turnId: agent.activeForegroundTurnId,
    } as AgentStreamEvent;
  }
