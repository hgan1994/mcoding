...
function buildPrompt(needs: AgentMetadataNeeds): string {
  ...
  if (needs.needsTitle) {
    instructions.push(`Title: short descriptive label (<= ${MAX_AUTO_AGENT_TITLE_CHARS} chars). Use the same language as the user prompt (e.g., Chinese prompt -> Chinese title).`);
  }
  if (needs.needsBranch) {
    instructions.push(
