...
export async function listDirectoryEntries({
  ...
  if (recursive && normalizedQuery.length > 0) {
  ...
  const entries = entriesWithNulls
...
async function searchDirectoryEntries({
  ...
  async function visit(currentPath: string, depth: number): Promise<void> {
    ...
    for (const dirent of dirents) {
      ...
      const targetPath = path.join(currentPath, dirent.name);
      const kind: ExplorerEntryKind = dirent.isDirectory() ? "directory" : "file";
      let payload: FileExplorerEntry;
      try {
        payload = await buildEntryPayload({
          root,
          targetPath,
          name: dirent.name,
          kind,
      ...
      const matches = directoriesOnly
        ? payload.kind === "directory" && lowerName.includes(query)
        : lowerName.includes(query) || payload.path.toLowerCase().includes(query);
      if (matches && (!directoriesOnly || payload.kind === "directory")) {
        results.push(payload);
      }

      if (payload.kind === "directory" && depth < maxDepth && !(directoriesOnly && matches)) {
