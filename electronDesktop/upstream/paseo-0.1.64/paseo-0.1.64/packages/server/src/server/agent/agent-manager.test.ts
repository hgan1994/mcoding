...
test("foreground turns settle when provider events omit turnId", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-missing-turnid-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);

  class MissingTurnIdSession extends TestAgentSession {
    private nextTurn = 0;

    override async startTurn(): Promise<{ turnId: string }> {
      const turnId = `turn-${++this.nextTurn}`;
      const reply = `reply ${this.nextTurn}`;
      setTimeout(() => {
        this.pushEvent({ type: "turn_started", provider: this.provider });
        this.pushEvent({
          type: "timeline",
          provider: this.provider,
          item: { type: "assistant_message", text: reply },
        });
        this.pushEvent({ type: "turn_completed", provider: this.provider });
      }, 0);
      return { turnId };
    }
  }

  class MissingTurnIdClient extends TestAgentClient {
    override async createSession(config: AgentSessionConfig): Promise<AgentSession> {
      return new MissingTurnIdSession(config);
    }
  }

  const manager = new AgentManager({
    clients: {
      codex: new MissingTurnIdClient(),
    },
    registry: storage,
    logger,
  ...
  const drainRun = async (prompt: string) => {
    const events: AgentStreamEvent[] = [];
    for await (const event of manager.streamAgent(snapshot.id, prompt)) {
      events.push(event);
    }
    return events;
  };
  ...
  const first = await timeout(drainRun("first"));
  ...
  const second = await timeout(drainRun("second"));
  expect(second).not.toBe("timeout");
  expect(second).toContainEqual(
    expect.objectContaining({ type: "turn_completed", turnId: "turn-2" }),
  );
  expect(manager.getAgent(snapshot.id)?.lifecycle).toBe("idle");
  expect(manager.getAgent(snapshot.id)?.activeForegroundTurnId).toBeNull();
});
