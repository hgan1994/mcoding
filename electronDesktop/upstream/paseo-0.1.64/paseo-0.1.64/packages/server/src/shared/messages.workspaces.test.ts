...
describe("workspace message schemas", () => {
  ...
  test("parses recursive directory search requests", () => {
    const parsed = SessionInboundMessageSchema.parse({
      ...
      requestId: "req-file-search",
    });

    expect(parsed.type).toBe("file_explorer_request");
    expect(parsed.searchQuery).toBe("paseo");
    expect(parsed.recursive).toBe(true);
    expect(parsed.directoriesOnly).toBe(true);
  });
