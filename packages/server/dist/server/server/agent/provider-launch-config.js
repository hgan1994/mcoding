...
export const ProviderOverrideSchema = z
    ...
    .strict();
export const AgentProviderRuntimeSettingsMapSchema = z.record(AgentProviderSchema, ProviderRuntimeSettingsSchema);
...
export function migrateProviderSettings(raw, builtinProviderIds) {
    ...
    for (const [providerId, value] of Object.entries(raw)) {
        ...
        if (command?.mode === "append") {
            console.warn(`[Config] Skipping legacy agents.providers.${providerId}.command append mode during provider override migration because it cannot be auto-migrated.`);
            continue;
        }
...
export function applyProviderEnv(baseEnv, runtimeSettings) {
    const merged = {
        ...baseEnv,
        ...(runtimeSettings?.env ?? {}),
    };
