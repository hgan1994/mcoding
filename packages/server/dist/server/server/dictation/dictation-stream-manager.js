...
import { createDictationDebugChunkWriter, maybePersistDictationDebugAudio, } from "../agent/dictation-debug.js";
...
const DICTATION_SILENCE_PEAK_THRESHOLD = Number.parseInt(process.env.PASEO_DICTATION_SILENCE_PEAK_THRESHOLD ?? "300", 10);
...
export class DictationStreamManager {
    ...
    async handleStart(dictationId, format) {
        this.cleanupDictationStream(dictationId);
        ...
        const debugChunkWriter = createDictationDebugChunkWriter({ sessionId: this.sessionId, dictationId }, this.logger);
        const outputRate = stt.requiredSampleRate;
        const autoCommitBytes = this.autoCommitSeconds > 0
    ...
    async handleChunk(params) {
        ...
        if (params.format !== state.inputFormat) {
            void this.failAndCleanupDictationStream(params.dictationId, `Mismatched dictation stream format: ${params.format}`, false);
            return;
        }
    ...
    async handleFinish(dictationId, finalSeq) {
        const state = this.streams.get(dictationId);
        if (!state) {
        ...
        if (finalSeq >= 0 &&
            ...
            this.failDictationStream(dictationId, `Dictation finished (finalSeq=${finalSeq}) but no audio chunks were received`, true);
            this.cleanupDictationStream(dictationId);
            return;
        }
        this.maybeSealDictationStreamFinish(dictationId);
        ...
        updatedState.finalTimeout = setTimeout(() => {
            void this.failAndCleanupDictationStream(dictationId, "Timed out waiting for final transcription", true);
    ...
    async maybePersistDictationStreamAudio(dictationId) {
        ...
        const wavBuffer = convertPCMToWavBuffer(pcmBuffer, state.outputRate, PCM_CHANNELS, PCM_BITS_PER_SAMPLE);
        const path = await maybePersistDictationDebugAudio(wavBuffer, { sessionId: state.sessionId, dictationId: state.dictationId, format: "audio/wav" }, this.logger, state.debugChunkWriter?.folder);
    ...
    async failAndCleanupDictationStream(dictationId, error, retryable) {
    ...
    cleanupDictationStream(dictationId) {
    ...
    estimateFinalizationTimeout(state) {
        ...
        const pendingUncommittedTranscriptSegments = Array.from(state.transcriptsBySegmentId.keys()).reduce((count, segmentId) => {
            if (committedSet.has(segmentId)) {
                return count;
            }
            return state.finalTranscriptSegmentIds.has(segmentId) ? count : count + 1;
        }, 0);
        const pendingSegments = pendingCommittedSegments +
        ...
        const missingSeqCount = state.finalSeq === null ? 0 : Math.max(0, state.finalSeq - state.ackSeq);
        const extraMs = pendingSegments * DICTATION_FINAL_TIMEOUT_PER_PENDING_SEGMENT_MS +
        ...
        const timeoutMs = Math.max(this.finalTimeoutMs, Math.min(DICTATION_FINAL_TIMEOUT_MAX_MS, this.finalTimeoutMs + extraMs));
        return {
            timeoutMs,
            pendingSegments,
            pendingAudioSeconds,
    ...
    maybeAutoCommitDictationSegment(state) {
    ...
    maybeSealDictationStreamFinish(dictationId) {
        const state = this.streams.get(dictationId);
        if (!state) {
            return;
        }
    ...
    maybeFinalizeDictationStream(dictationId) {
        const state = this.streams.get(dictationId);
        if (!state) {
            return;
        }
        ...
        const allTranscriptsReady = orderedSegmentIds.every((segmentId) => state.finalTranscriptSegmentIds.has(segmentId));
...
function isBufferTooSmallError(message) {
    return /buffer too small/i.test(message);
}
//# sourceMappingURL=dictation-stream-manager.js.map