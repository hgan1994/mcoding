...
export async function listDirectoryEntries({ root, relativePath = ".", }) {
    ...
    const entriesWithNulls = await Promise.all(dirents.map(async (dirent) => {
    ...
    const entries = entriesWithNulls.filter((entry) => entry !== null);
    entries.sort((a, b) => {
...
export async function readExplorerFile({ root, relativePath, }) {
