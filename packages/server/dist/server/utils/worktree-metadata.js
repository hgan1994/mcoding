...
export function getPaseoWorktreeMetadataPath(worktreeRoot) {
    const gitDir = getGitDirForWorktreeRoot(worktreeRoot);
    return join(gitDir, "paseo", "worktree.json");
}
export function normalizeBaseRefName(input) {
...
export function writePaseoWorktreeMetadata(worktreeRoot, options) {
...
export function requirePaseoWorktreeBaseRefName(worktreeRoot) {
    const metadataPath = getPaseoWorktreeMetadataPath(worktreeRoot);
